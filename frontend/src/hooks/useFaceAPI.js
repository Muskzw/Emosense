...
export function useFaceAPI(videoRef, svgRef, canvasRef, isConnected, sessionCtx) {
  ...
  useEffect(() => {
    ...
    // Reset timeline counters for a fresh call
    timelineRef.current  = [];
    ...
    const loop = async (time) => {
      ...
      // Ref checks are inside loop so we keep running while video loads
      const video = videoRef.current;
      const svg   = svgRef.current;
      if (!video || video.videoWidth === 0) return; // video not ready yet
      ...
      try {
        ...
        if (!det) return; // no face in frame — keep looping

        // Map to our 4-emotion vocabulary
